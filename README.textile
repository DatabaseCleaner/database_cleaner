...
Add **Redis** and **Ohm** orm support which uses Truncation strategy.
...
(with orm Ohm and Redis, :only and :except accept options like "Widget:Name*" or "*:id*" that redis command KEYS accept)
...
| Redis         | DatabaseCleaner[:redis]         | Connection specified as redis url                                        |
